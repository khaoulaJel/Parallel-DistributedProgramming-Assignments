...
void dmvm_v2(int n, int m, double *lhs, double *rhs, double *mat) {
    #pragma omp parallel
    {
        // dynamic: threads grab small chunks of work as they become free
        // nowait: threads don't wait for others at the end of the loop
        #pragma omp for schedule(dynamic) nowait reduction(+:lhs[:m])
        for (int c = 0; c < n; ++c) {
            int offset = m * c;
            for (int r = 0; r < m; ++r) {
...
int main() {
    ...
    printf("Version 2 (Dynamic + Nowait) Time: %.6f s\n", end - start);
